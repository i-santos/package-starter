...
- `packages/release-cli`: CLI CommonJS com os comandos `beta`, `stable` e `registry`.
...
- `npm run release:beta`
  - exige git limpo
  - executa bump prerelease beta
  - commit automático `chore(release): vX.Y.Z-beta.N`
  - publica com `npm publish --tag beta`
- `npm run release:stable`
  - exige git limpo
  - se versão atual for beta, promove para `X.Y.Z`
  - se já for stable, faz bump patch
  - commit automático `chore(release): vX.Y.Z`
