...
test('release-cli beta rolls back version and commit when publish fails', () => {
  const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'release-rollback-'));
  ...
  const beta = run('node', [binPath, 'beta'], workDir);
  assert.notEqual(beta.status, 0, 'beta should fail without a registry');

  const pkg = JSON.parse(fs.readFileSync(path.join(workDir, 'package.json'), 'utf8'));
  assert.equal(pkg.version, '1.0.0', 'version should be rolled back');

  const status = run('git', ['status', '--porcelain'], workDir);
  assert.equal(status.status, 0);
  assert.equal(status.stdout.trim(), '', 'git should stay clean after rollback');

  const afterLog = run('git', ['rev-list', '--count', 'HEAD'], workDir);
  assert.equal(afterLog.status, 0);
  assert.equal(afterLog.stdout.trim(), beforeLog.stdout.trim(), 'no release commit should be created');
});
